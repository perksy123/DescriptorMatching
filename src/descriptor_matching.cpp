                      ...
                      vector<cv::DMatch> &matches, string descriptorType, string matcherType, string selectorType)
{
    ...
    { // k nearest neighbors (k=2)
        ...
        {
            // Only accept if the descriptor distance ratio is lower (???) than the above tolerance
            if ((*it)[0].distance < minDescriptorDistanceRatio * (*it)[1].distance)
            {
                matches.push_back((*it)[0]);
            } 
        }
        std::cout << "Keypoints removed = " << static_cast<double>(kMatches.size() - matches.size()) / kMatches.size() << std::endl;
    }
